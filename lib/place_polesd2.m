function KySq = place_polesd(p,px,ni,wp,ws,as,e_,type)
%   Kz_ = place_polesd(p,ni,w,e_,type,px)
...
for i = 1:2000 % repeat enough times to guarantee success 
    Kz_ = make_Kz(p,px_,type);
    KySq = make_KySq(p,px_,type);
    Kz2 = Kz_*Kz_';
    zmin1 = find_minimad(Kz_,ws,as,wp); % find the minima of the stop-band loss
    zmin = find_minimad2(KySq,ws,as,wp); % find the minima of the stop-band loss
    ...
    Y1 = -find_margind(Kz_,ws,as,wp,e_,zmin);
    Y = -find_margin3d(KySq,ws,as,wp,zmin);
   
    S = [dlogKK_dp2(KySq,px_,zmin) s2]; % Calculate the sensitivity matrix
...
zmin = find_minimad(Kz_,ws,as,wp); % find the minima of the stop-band loss
if nx > 0
    if ~any(px_ < p(1))
        zmin = [wsz(1) zmin];
    end
