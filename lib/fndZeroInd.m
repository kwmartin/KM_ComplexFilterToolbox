function indc = fndZeroInd(f,deriv,p)
%   indc = fndZeroInd(deriv,p) finds the zero-crossings of the derivative function used in
...
%
    ...
    fpi = angle(p)./(2*pi);
    fi = 1.01*[fpi(1) fpi(end)];
    ind1 = min(find(abs(fi(1) - f) < 1e-5));
    ind2 = min(find(abs(fi(2) - f) < 1e-5));
    f2 = f(ind1:ind2);
