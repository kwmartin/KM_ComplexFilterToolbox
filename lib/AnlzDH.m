function [lgH, phH, gdH, dLdW, dTdW, d2LdW, d2TdW] = AnlzDH(H, w)
...
%
  ...
  for l = 1:nz
      ...
      dddrvSum = dddrvSum + (eS.*z(l).*(eS+z(l)))./((eS - z(l)*onesVctr).^3);
  end
  %gdv = zeros(np, npts);
  for l = 1:np
      ...
      dddrvSum = dddrvSum - (eS.*p(l).*(eS+p(l)))./((eS - p(l)*onesVctr).^3);
  end
