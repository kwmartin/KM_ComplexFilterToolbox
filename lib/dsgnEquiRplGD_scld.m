function [H, p, px, wp, ws, as] = dsgnEquiRplGD_scld(p,px,wp,ws,as,Ap,Ordr,sclFctr,shftFctr,deltGD)
%   [_scld copy of dsgnEquiRplGD.m - calls LinPh_LssPls_scld, fndZeroCrs3_scld and adaptP3_scld]
% H = dsgnEquiRplGD_scld(p,px,wp,ws,as,Ap,Ordr,deltGD) designs a complex digital filter
...
%
    ...
    % adaptP3_scld/fndZeroCrs3_scld search around H3's actual passband.
    wp3 = wp + shftFctr;

    % Check group-delay extrema BEFORE calling adaptP3_scld: how many are
    % there, and how close to equi-ripple already? adaptP3_scld is only meant
    ...
    % it's not needed (confirmed on EqualFltr_1_6_0.m: the pre-adaptP3_scld
    % group delay was already at 10.2% p2p/mean ripple with a clean
    % alternating extrema pattern, and adaptP3_scld's own pole-angle-warping
    ...
    wzPre = fndZeroCrs3_scld(H3, wp3);
    if length(wzPre) < MIN_EXTREMA_TO_ASSESS
        error('dsgnEquiRplGD_scld:tooFewExtrema', ...
            ['dsgnEquiRplGD_scld: only found %d group-delay extrema in the ' ...
    ...
    if ripplePre < RIPPLE_SKIP_THRESHOLD
        fprintf(['dsgnEquiRplGD_scld: group delay already within %.1f%% of ' ...
            'equi-ripple (%.2f%%) - skipping adaptP3_scld\n'], ...
            100*RIPPLE_SKIP_THRESHOLD, 100*ripplePre);
        H4 = H3;
    else
        % adaptP3_scld can itself fail outright (e.g. it can't find enough
        ...
        % it the same as "adaptP3_scld made things worse" and revert.
        try
            H4 = adaptP3_scld(H3,0.1,wp3);
            wzPost = fndZeroCrs3_scld(H4, wp3);
            ...
            if revert
                if length(wzPost) < MIN_EXTREMA_TO_ASSESS
                    fprintf(['dsgnEquiRplGD_scld: adaptP3_scld''s output has too few ' ...
                        'extrema to assess (%d) - reverting to pre-adaptP3_scld ' ...
                        'poles\n'], length(wzPost));
                else
                    fprintf(['dsgnEquiRplGD_scld: adaptP3_scld made group-delay ripple ' ...
                        'worse (%.2f%% -> %.2f%%) - reverting to pre-adaptP3_scld ' ...
                        'poles\n'], 100*ripplePre, 100*ripplePost);
                end
                H4 = H3;
            end
        catch ME
            fprintf(['dsgnEquiRplGD_scld: adaptP3_scld failed (%s) - reverting to ' ...
                'pre-adaptP3_scld poles\n'], ME.message);
            H4 = H3;
        end
    end
