function zmin = find_minimad2(sys,ws,as,wp)
%   zmin = find_minimad2(sys,ws,as,wp) finds the stop-band attenuation minima
...
for k = 1:np_-1
    dw = (p_(k + 1) - p_(k))/NPTS;
    w = p_(k):dw:p_(k + 1);
    m10 = find_margin3d(sys,ws,as,wp,w);
    [min1 i] = min(m10);
    wmin = w(i);
    if min1 < find_margin3d(sys,ws,as,wp,winit(k))
        winit(k) = wmin;
    end
end
zmin = winit.';
