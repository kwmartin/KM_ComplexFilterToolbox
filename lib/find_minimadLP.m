function zmin = find_minimadLP(sys,ws,as,wp)
%   zmin = find_minimadLP(sys,ws,as,wp) finds the stop-band attenuation minima
%   This is a simple program for finding the minima of the stopband of abs(Hy)
%   in the y plane
%   The minima of |Hy| are all assumed to be between the loss poles.
%   Loss poles at 0 in y correspond to e^(jw2) in z
%   Loss poles at Inf in y correspond to e^(jw1) in z
%   Loss poles at j in y correspond to e^(j*pi) in z
%   All loss poles are on positive jw axis
%
...
for k = 1:np_-1
    dw = (p_(k + 1) - p_(k))/NPTS;
    w = p_(k):dw:p_(k + 1);
    m10 = find_margin2d(sys,ws,as,wp,w);
    [min1 i] = min(m10);
    wmin = w(i);
    if min1 < find_margin2d(sys,ws,as,wp,winit(k))
        winit(k) = wmin;
    end
end
zmin = winit.';
