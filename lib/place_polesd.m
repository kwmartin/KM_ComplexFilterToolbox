function Kz_ = place_polesd(p,px,ni,wp,ws,as,e_,type)
...
Kz_ = make_init2Kzd(p,px,ni,wp,type); % Calculate the initial characteristic eq.
...
for i = 1:2000 % repeat enough times to guarantee success 
    ...
    % Y = -find_margind(Kz_,ws,as,wp,e_,zmin);
    Y = -find_margin2d(Kz_,ws,as,wp,zmin);
