...
%
    ...
    p1 = bestP1;
    fprintf(['adaptP3: starting candidates refined - original reached %d, tan-warp ' ...
    ...
    if useReduced
        fprintf(['adaptP3: poles are conjugate-paired and %d maxima match %d poles - ' ...
        ...
        try
            ...
            % back to the caller's own pre-adaptP3 poles at that point
            ...
            % elsewhere.
            fprintf(['adaptP3: reduced Newton system failed (%s) - falling back to ' ...
                'standard Newton system\n'], ME.message);
            [H, w, f] = adaptP3Standard(z, k, p1, Np, wp, deltT, wz, R_MAX);
        end
    else
        if ~isPaired
            fprintf('adaptP3: poles are not conjugate-paired - using standard Newton system\n');
        else
            fprintf(['adaptP3: %d maxima found, does not match %d poles - using standard ' ...
                'Newton system\n'], nMaxFound, Np);
        end
        [H, w, f] = adaptP3Standard(z, k, p1, Np, wp, deltT, wz, R_MAX);
    end
...
%   not just by count (see adaptP3's own comments above its candidate
...
%   Guaranteed by construction for every current adaptP3 caller
%   (dsgnEquiRplGD.m re-centers to baseband before calling adaptP3), but
...
%   this; nothing upstream (fndZeroCrs3, setY2) previously used it -
...
%   point (see adaptP3's own comments on why the standard system is
%   near-singular).
    Np = double(Np);
    nPairs = floor(Np/2);
...
%   Reduced conjugate-pair Newton loop - see adaptP3's own comments
...
%   validation numbers.
    ...
    for i = 1:N
        ...
        if length(wzMax) ~= Np || isempty(wzMin)
            ...
            fprintf(['adaptP3 (reduced): maxima/minima structure changed (%d maxima, ' ...
            ...
            if structureBadIters > STRUCTURE_MAX_BAD_ITERS
                error('adaptP3:reducedStructureLost', ...
                    ['adaptP3 (reduced): maxima/minima structure changed (found %d maxima, ' ...
...
%   whenever adaptP3Reduced's preconditions don't hold. See adaptP3's
