function H = adaptP3_scld(H, deltT, wp)
%   [_scld copy of adaptP3.m - calls fndZeroCrs3_scld]
%   [H2, deltT] = adaptP3_scld(H, deltT, wp) adapts the poles of a digital filter to correct the group delay
...
%
    ...
    p1 = bestP1;
    fprintf(['adaptP3_scld: starting candidates refined - original reached %d, tan-warp ' ...
    ...
    if bestCount < 2*Np - 1
        ...
        % their pre-adaptP3_scld state (same principle as this file's own
        % in-loop collision-halt errors below).
        error('adaptP3_scld:tooFewStartingExtrema', ...
            ['adaptP3_scld: could not reach %d starting extrema after both ' ...
    ...
    if useReduced
        fprintf(['adaptP3_scld: poles are conjugate-paired and %d maxima match %d poles - ' ...
        ...
        try
            ...
            % back to the caller's own pre-adaptP3_scld poles at that point
            ...
            % elsewhere.
            fprintf(['adaptP3_scld: reduced Newton system failed (%s) - falling back to ' ...
                'standard Newton system\n'], ME.message);
            [H, w, f] = adaptP3Standard(z, k, p1, Np, wp, deltT, wz, R_MAX);
        end
    else
        if ~isPaired
            fprintf('adaptP3_scld: poles are not conjugate-paired - using standard Newton system\n');
        else
            fprintf(['adaptP3_scld: %d maxima found, does not match %d poles - using standard ' ...
                'Newton system\n'], nMaxFound, Np);
        end
        [H, w, f] = adaptP3Standard(z, k, p1, Np, wp, deltT, wz, R_MAX);
    end
...
%   see adaptP3_scld's own comments above its call to this function for the
%   full rationale.
    p1 = m.*exp(j*aInit);
    H1 = zpk(z, p1, k);
    wz = fndZeroCrs3_scld(H1, wp);
    ...
    while bestCount < 2*Np - 1 && iter < maxIters
        ...
        kA = srt(worstIdx);
        kB = srt(worstIdx+1);
        fprintf(['adaptP3_scld (%s): short %d extrema (found %d, best %d, need ' ...
...
%   not just by count (see adaptP3_scld's own comments above its candidate
...
%   Guaranteed by construction for every current adaptP3_scld caller
%   (dsgnEquiRplGD.m re-centers to baseband before calling adaptP3_scld), but
...
%   this; nothing upstream (fndZeroCrs3_scld, setY2) previously used it -
...
%   point (see adaptP3_scld's own comments on why the standard system is
%   near-singular).
    Np = double(Np);
    nPairs = floor(Np/2);
...
%   Reduced conjugate-pair Newton loop - see adaptP3_scld's own comments
...
%   validation numbers.
    ...
    for i = 1:N
        ...
        if length(wzMax) ~= Np || isempty(wzMin)
            ...
            fprintf(['adaptP3_scld (reduced): maxima/minima structure changed (%d maxima, ' ...
            ...
            if structureBadIters > STRUCTURE_MAX_BAD_ITERS
                error('adaptP3_scld:reducedStructureLost', ...
                    ['adaptP3_scld (reduced): maxima/minima structure changed (found %d maxima, ' ...
...
%   whenever adaptP3Reduced's preconditions don't hold. See adaptP3_scld's
...
%   the fallback rather than replaced outright.
    ...
    % happens, fndZeroCrs3_scld legitimately finds fewer genuine group-delay
    ...
    for i = 1:N
        ...
        if npFree > 1
            ...
            for mm = 1:(length(sortedFree)-1)
                ...
                if abs(p(kB) - p(kA)) < COLLISION_TOL
                    fprintf(['adaptP3_scld: poles %d and %d drifted to nearly the ' ...
        ...
        if npts + 1 < 2*npFree
            ...
            % fndZeroCrs3_scld already assume elsewhere in this file) and
            % freeze one of them.
            [~, sortIdx] = sort(angle(p(freeIdx)));
            sortedFree = freeIdx(sortIdx);
            for mm = 1:(length(sortedFree)-1)
                ...
                kB = sortedFree(mm+1);
                if abs(p(kB) - p(kA)) < COLLISION_TOL
                    fprintf(['adaptP3_scld: poles %d and %d converged to nearly the ' ...
                        'same position (%.4g%+.4gi, %.4g%+.4gi) - freezing ' ...
                        'pole %d\n'], kA, kB, real(p(kA)), imag(p(kA)), ...
                        real(p(kB)), imag(p(kB)), kB);
                    frozen(kB) = true;
            ...
            if collisionIters > COLLISION_MAX_ITERS
                error('adaptP3_scld:tooFewExtrema', ...
                    ['adaptP3_scld: only found %d group-delay extrema for %d free ' ...
