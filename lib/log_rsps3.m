function [hdb h] = log_rsps3(sys,s)
%   [hdb h] = log_rsps3(sys,s)calculates the log10 response of a ZySq system.
%   It replaces s by s^2 assumiong ZySq was obtained using an s' = s^2
%   transform.
...
for i = 1:ls % Iterate for each frequency
    zs = s(i)^2 - z{1}; % Calculate the vector of zeros; z is array of coefficients possibly complex
    indxz = (zs == 0);
    zs(logical(indxz)) = 10*eps;
    ps = s(i)^2 - p{1};% Calculate the vector of poles
    indxp = (ps == 0);
